...
import { createUsers, readUsers, updateUser } from "../../service/user/user.service.js";
import { generateToken, hashValue, verifyValue } from "../../utils/auth.utils.js";
import { httpCodeUtils, responseUtils } from "../../utils/response.utils.js";

export const signup = errorWrapper(async (req, res) => {
    const user = await readUsers({ email: req.body.email }, {});
...
export const login = errorWrapper(async (req, res) => {
    ...
    const user = await readUsers({ email: req.body.email, isDeleted: false, isAdmin: isAdmin }, {});
    if (user && user.count === 1) {
        const isCorrect = await verifyValue(req.body.password, user.docs[0].password);
        if (isCorrect) {
